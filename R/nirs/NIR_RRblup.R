# *******************************************************************
# Procedimento:  Seleção Genômica Ampla - RR-BLUP
...
# *******************************************************************
# 3. Leitura do arquivo de dados fenotípicos
# *******************************************************************
...
valorAIC <- matrix(rep(NA),ncol(dados)-1,valork)
...
# *******************************************************************
fit2=mixed.solve(variavel, Z=M1, K=diag(ncol(M1)))

cat("Efeito dos marcadores","\n")
print(fit2$u)

par(mfrow=c(3,1))  # divide a tela gráfica em 3
seq <-(1:1:length(fit2$u))

plot(seq,fit2$u,xlab="Marcas",ylab="Efeito",main = colnames(fenotipo)[i], type = "l")
plot(seq,abs(fit2$u),xlab="Marcas",ylab="Abs(Efeito)",type = "l")
# visualização dos mais informativos 
plot(density(fit2$u))

# Marcadores ordenadas pela importância
cat("  ", "\n")
cat("Ondas ordenados pela importância ","\n")
print(order(abs(fit2$u), decreasing=TRUE))

# *******************************************************************
# 4.3 Vetor de valores genômicos
# *******************************************************************
a=as.matrix(fit2$u)
rownames(a)<-c(colnames(nir))
gbv_rr= M1%*%a 
cat("  ", "\n")
...
r2= cor(gbv_rr,variavel)^2
cat("  ", "\n")
cat("R²(Vpred,Média) =", r2, "\n")
valorR2t[i-1,kkuu] <- r2

 erro<-gbv_rr-variavel
  erro2<-erro^2
  soma<-sum(erro2)
  nn<-length(erro)
...
AIC = -2*fit2$LL + 2*ncol(M1)
valorAIC[i-1,kkuu] <- AIC
cat("  ", "\n")
cat("AIC =", AIC, "\n")
...
rownames(gbv_rr)<-c(dados$id[1:nindTeste])

#ordenamento dos valores genômicos
gbv1=sort(as.matrix(gbv_rr)[,],decreasing=TRUE) #ordenamento dos valores
plot(hist(gbv1))
...
par(mfrow=c(3,1))  # divide a tela gráfica em 3
plot(seq,gbv_rr,xlab="Ind - Teste",ylab="Vpred", main = colnames(fenotipo)[i], type = "l")
plot(seq,variavel,xlab="Ind - Teste",ylab="Média fen",  type = "l")
plot(variavel,gbv_rr,xlab="Média fen",ylab="Vpred")
...
r2= cor(gbv_rr_val,variavelval)^2
cat("  ", "\n")
cat("R²(Vpred,Média) =", r2, "\n")
valorR2v[i-1,kkuu] <- r2


 erro<-gbv_rr_val-variavelval
  erro2<-erro^2
  soma<-sum(erro2)
  nn<-length(erro)
  reqmv<-sqrt((soma/nn))
...
par(mfrow=c(3,1))  # divide a tela gráfica em 3
plot(seq,gbv_rr_val,xlab="Ind - Validação",ylab="Vpred", main = colnames(fenotipo)[i], type = "l")
plot(seq,variavelval,xlab="Ind - Validação",ylab="Média fen",  type = "l")
plot(variavelval,gbv_rr_val,xlab="Média fen",ylab="Vpred")

# *******************************************************************
# 4.5 Estimativas do Modelo RR-BLUP
# *******************************************************************

# Capacidade Preditiva
seq <-(1:1:length(variavelval))
CP= cor(gbv_rr_val,variavelval)
#cat("  ", "\n")
...
# Viés
seq <-(1:1:length(variavel))
beta1= ((cov(gbv_rr,variavel))/(var(variavel)))
#cat("  ", "\n")
...
res.par <- cbind(valorR2t,valorR2v, valorCP, valorbeta1, valorerrot, valorerrov, valorAIC, valorR2ajt, valorR2ajv)
nvar = ncol(dados)-1
