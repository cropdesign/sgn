# *******************************************************************
# Procedimento: GWS- Regressão via Quadrados Mínimos Parciais
...
cat("**                                                      **  ", "\n")
cat("**  NIR PLS Regressão via Quadrados Mínimos Parciais    **" ,"\n")
cat("**********************************************************", "\n")
...
#********************************************
#   M matriz de espetro       fenotipo: matriz de dados
#********************************************
...
cat("======================================================","\n")
cat("  ", "\n")
cat("(*) Identificar o número de componentes que explicam 80% da variação da variável   ", "\n")
cat("Número de componentes estabelecidos pelo usuário  = ", ndim, "\n")
cat("  ", "\n")


pls=plsr(variavel~M1)
names(pls)
...
# *******************************************************************
# 4.2 Vetor de efeito dos espectros
# *******************************************************************
nc_pls=ndim

# coeficientes associados as marcas head(m_pls)
m_pls=pls$coefficients[,,nc_pls]


cat("Efeito dos marcadores = PSLR","\n")
print(m_pls)

par(mfrow=c(3,1))  # divide a tela gráfica em 3
seq <-(1:1:length(m_pls))

plot(seq,m_pls,xlab="Onda",ylab="Efeito",main = colnames(fenotipo)[i], type = "l")
plot(seq,abs(m_pls),xlab="Onda",ylab="Abs(Efeito)",type = "l")
# visualização dos mais informativos 
plot(density(m_pls))

# Marcadores ordenados pela importância
cat("  ", "\n")
cat("Ondas ordenadas pela importância ","\n")
print(order(abs(m_pls), decreasing=TRUE))

# *******************************************************************
# 4.3 Vetor de valores preditos
# *******************************************************************

gbv_pls=M1%*%m_pls # valor predito (GEBVs) dos indivíduos



cat("  ", "\n")
cat("Valores Preditors VPred ","\n")
print(gbv_pls)


# R2 vfen e vgenômico
seq <-(1:1:length(variavel))
r2= cor(gbv_pls,variavel)^2
cat("  ", "\n")
cat("R²(VPred,Média) =", r2, "\n")
valorR2t[i-1,kkuu] <- r2


  erro<-gbv_pls-variavel
  erro2<-erro^2
  soma<-sum(erro2)
  nn<-length(erro)
...
rownames(gbv_pls)<-c(dados$id[1:nindTeste])

#ordenamento dos valores preditos
gbv1=sort(as.matrix(gbv_pls)[,],decreasing=TRUE) #ordenamento dos valores
plot(hist(gbv1))
...
par(mfrow=c(3,1))  # divide a tela gráfica em 3
plot(seq,gbv_pls,xlab="Ind - Teste",ylab="VPred", main = colnames(fenotipo)[i], type = "l")
plot(seq,variavel,xlab="Ind - Teste",ylab="Média fen",  type = "l")
plot(variavel,gbv_pls,xlab="Média fen",ylab="VPred")
...
# *******************************************************************
gbv_pls_val=as.matrix(M2)%*%m_pls
cat("  ", "\n")
...
r2= cor(gbv_pls_val,variavelval)^2
...
par(mfrow=c(3,1))  # divide a tela gráfica em 3
plot(seq,gbv_pls_val,xlab="Ind - Validação",ylab="VPred", main = colnames(fenotipo)[i], type = "l")
plot(seq,variavelval,xlab="Ind - Validação",ylab="Média fen",  type = "l")
plot(variavelval,gbv_pls_val,xlab="Média fen",ylab="VPred")
...
seq <-(1:1:length(variavelval))
CP= cor(gbv_pls_val,variavelval)
#cat("  ", "\n")
...
# Viés
seq <-(1:1:length(variavel))
beta1= ((cov(gbv_pls,variavel))/(var(variavel)))
#cat("  ", "\n")
