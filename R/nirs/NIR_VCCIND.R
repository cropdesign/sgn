# *******************************************************************
# Procedimento:  Seleção Genômica Ampla - Componentes Independentes
...
cat("**                                                      **  ", "\n")
cat("**     Procedimento: NIR Componentes Independentes      **" ,"\n")
cat("**********************************************************", "\n")
...
cat("======================================================","\n")
cat("  ", "\n")
cat("(*) Identificar o número de componentes que explica 80% da variação de X por componentes principais  ", "\n")
cat("Número de componentes estabelecidos pelo usuário  = ", ndim, "\n")
cat("  ", "\n")


icr=icr(M1,variavel,n.comp=ndim)
names(icr)
names(icr$ica$ica)
...
beta_icr=icr$model$coefficients[-1] # coeficientes associdos aos componentes
m_icr=Ainv_icr%*%beta_icr # vetor de efeitos estimados SNPs




cat("Efeito das ondas = icr","\n")
print(m_icr)

par(mfrow=c(3,1))  # divide a tela gráfica em 3
seq <-(1:1:length(m_icr))

plot(seq,m_icr,xlab="Marcas",ylab="Efeito",main = colnames(fenotipo)[i], type = "l")
plot(seq,abs(m_icr),xlab="Ondas",ylab="Abs(Efeito)",type = "l")
# visualização dos mais informativos 
plot(density(m_icr))

# Comprimento de ondas ordenadas pela importância
cat("  ", "\n")
cat("Comprimento de ondas ordenadas pela importância ","\n")
print(order(abs(m_icr), decreasing=TRUE))

# *******************************************************************
# 4.3 Vetor de valores genômicos
# *******************************************************************

gbv_icr=M1%*%m_icr # valor genético genômico (GEBVs) dos indivíduos
...
r2= cor(gbv_icr,variavel)^2
cat("  ", "\n")
cat("R²(Vpred,Média) =", r2, "\n")
valorR2t[i-1,kkuu] <- r2


  erro<-gbv_icr-variavel
  erro2<-erro^2
  soma<-sum(erro2)
  nn<-length(erro)
...
rownames(gbv_icr)<-c(dados$id[1:nindTeste])

#ordenamento dos valores genômicos
gbv1=sort(as.matrix(gbv_icr)[,],decreasing=TRUE) #ordenamento dos valores
plot(hist(gbv1))
...
par(mfrow=c(3,1))  # divide a tela gráfica em 3
plot(seq,gbv_icr,xlab="Ind - Teste",ylab="Vpred", main = colnames(fenotipo)[i], type = "l")
plot(seq,variavel,xlab="Ind - Teste",ylab="Média fen",  type = "l")
plot(variavel,gbv_icr,xlab="Média fen",ylab="VPred")




# *******************************************************************
# 4.4 Vetor de valores genômicos - dados de validação
# *******************************************************************
gbv_icr_val=M2%*%m_icr
cat("  ", "\n")
...
r2= cor(gbv_icr_val,variavelval)^2
...
par(mfrow=c(3,1))  # divide a tela gráfica em 3
plot(seq,gbv_icr_val,xlab="Ind - Validação",ylab="Vpred", main = colnames(fenotipo)[i], type = "l")
plot(seq,variavelval,xlab="Ind - Validação",ylab="Média fen",  type = "l")
plot(variavelval,gbv_icr_val,xlab="Média fen",ylab="VPred")
...
seq <-(1:1:length(variavelval))
CP= cor(gbv_icr_val,variavelval)
#cat("  ", "\n")
cat("CP(EGBV,Média) =", CP, "\n")
valorCP[i-1,kkuu] <- CP

# Viés
seq <-(1:1:length(variavel))
beta1= ((cov(gbv_icr,variavel))/(var(variavel)))
#cat("  ", "\n")
cat("Beta1 - Cov(EGBV,Média)/var(variavel) =", beta1, "\n")
...
cat("**            Programa Genes - Programa R               **", "\n")
cat("**                                                      **  ", "\n")
cat("**         NIR-Componentes Independentes                **" ,"\n")
cat("**********************************************************", "\n")
