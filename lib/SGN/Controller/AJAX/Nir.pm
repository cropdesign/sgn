...
package SGN::Controller::AJAX::Nir;
...
sub shared_phenotypes: Path('/ajax/Nir/shared_phenotypes') : {
    my $self = shift;
    my $c = shift;
    ...
    $c->tempfiles_subdir("Nir_files");
    my ($fh, $tempfile) = $c->tempfile(TEMPLATE=>"Nir_files/trait_XXXXX");
...
sub extract_trait_data :Path('/ajax/Nir/getdata') Args(0) {
    my $self = shift;
    my $c = shift;
    ...
    my $temppath = File::Spec->catfile($c->config->{basepath}, "/tmp/vagrant/SGN-site/Nir_files/".$file);
...
sub generate_results: Path('/ajax/Nir/generate_results') : {
    my $self = shift;
    my $c = shift;
    my $dataset_id = $c->req->param('dataset_id');
    ...
    $c->tempfiles_subdir("Nir_files");
    my $Nir_tmp_output = $c->config->{cluster_shared_tempdir}."/Nir_files";
    mkdir $Nir_tmp_output if ! -d $Nir_tmp_output;
    ...
    my $temppath = $Nir_tmp_output . "/" . $tempfile;
   ...
   # my $cmd = "Rscript " . $c->config->{basepath} . "/R/Nir/2_blup_rscript.R " . $pheno_filepath . " " . $trait_id;
   # system($cmd);
    eval {
    my $cmd = {
            backend => $c->config->{backend},
            temp_base => $c->config->{cluster_shared_tempdir} . "/Nir_files",
    ...
    $cmd->run_cluster(
            "Rscript ",
            $c->config->{basepath} . "/R/Nir/ammi_rscript.R",
            $pheno_filepath
    );
    $cmd->alive;
    $cmd->is_cluster(1);
